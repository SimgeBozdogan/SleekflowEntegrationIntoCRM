...
function filterConversationsByZohoLead(conversations) {
    ...
    console.log('🔍 İsim bazlı filtreleme başlıyor:', {
        zohoName: zohoData.name,
        totalConversations: conversations.length
    });
    
    // Eğer isim yoksa, filtreleme yapma
    if (!zohoData.name || !zohoData.name.trim()) {
        console.log('⚠️ Zoho data\'da isim yok, filtreleme yapılamıyor');
        return conversations;
    }
    ...
    const zohoNameNormalized = normalizeName(zohoData.name);
    console.log('🔍 Zoho ismi normalize edildi:', {
        original: zohoData.name,
        normalized: zohoNameNormalized
    });
...
function renderConversations() {
    ...
    // Eğer showAllConversations = false ama Zoho lead konuşmaları varsa, onları göster + buton ekle
    if (!state.showAllConversations && state.filterByZohoLead && state.conversations && state.conversations.length > 0) {
        // Zoho lead konuşmalarını göster
        state.conversations.forEach(conv => {
            const item = document.createElement('div');
            item.className = 'conversation-item';
            if (state.currentConversation && state.currentConversation.id === conv.id) {
...
function updateLeadFilterInfo() {
    ...
    const leadLabel = zoho.name || zoho.phone || zoho.email || 'Bu lead';
    ...
    infoEl.innerHTML = `
        ...
        <button class="btn btn-secondary" id="showAllConversationsInline">
            Tüm konuşmaları göster
        </button>
    `;
...
(function() {
    ...
    if (typeof window !== 'undefined') {
        ...
        console.log('✅ Zoho lead data event listener eklendi (window ve document)');
        ...
        // Hemen kontrol et
        checkAndFilterExistingZohoData();
        
        // Birkaç kez daha kontrol et (konuşmalar yüklenene kadar)
